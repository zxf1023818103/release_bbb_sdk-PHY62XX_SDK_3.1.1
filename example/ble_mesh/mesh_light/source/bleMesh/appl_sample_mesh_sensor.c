...
API_RESULT UI_Sensor_servel_model_sensorStatus_get(UINT16 state_t, UINT16 id, uint8 *param, UINT16 *p_len)
{
    ...
    uint8 humidity_data[2] = { sht30_humidity_integer, 0 };
    uint8 temperature_data[2] = { sht30_temperature_integer, 0 };
    ...
    case MS_STATE_SENSOR_DATA_PROPERTY_ID_T:
    {
        ...
        case TEMPERATURE_PID:
        {
            //format A:
            data_len=0x02;
            *param = ((TEMPERATURE_PID & 0x07) << 5) | (data_len << 1);
            *(param + 1) = (TEMPERATURE_PID >>3) & 0XFF;
            osal_memcpy(&param[2], temperature_data, 2);
    ...
    case MS_STATE_SENSOR_DATA_T:
    {
        ...
        *(param + sensor_index) = (uint8)(HUMIDITY_PID >> 8);
        ...
        data_len = 2;
        *(param + sensor_index) = ((TEMPERATURE_PID & 0x07) << 5) | (data_len << 1);
        
        sensor_index++;
        *(param + sensor_index) = (TEMPERATURE_PID >> 3) & 0XFF;
        
        sensor_index++;
        osal_memcpy(&param[sensor_index], temperature_data, 2);
...
static API_RESULT UI_Sensor_servel_model_column_get(MS_ACCESS_SENSOR_MODEL_STATE_PARAMS *state_param, UINT16 id, uint8 *param, UINT16 *p_len)
{
    ...
    case TEMPERATURE_PID:
    {
        for (i = 0; i < TEMP_COLUMN_NUM; i++)
        {
            Value_x_Base = UI_sensor_search_column_value_x(UI_Temp_Sensor_series_column[i].sensor_raw_value_x,
                                                           UI_Temp_Sensor_series_column[i].sensor_raw_value_x_len);
            if (Value_x_Base != 0 && Value_x_Base == Value_x_Identify)
            {
                column_index = i;
                break;
        ...
        if (Value_x_Base == 0)
            break;
        index = 0;
        *(param + index) = (uint8)TEMPERATURE_PID;
        index++;
        *(param + index) = (uint8)(TEMPERATURE_PID >> 8);
        index++;

        osal_memcpy((uint8 *)param[index], (uint8 *)UI_Temp_Sensor_series_column[column_index].sensor_raw_value_x,
                    UI_Temp_Sensor_series_column[column_index].sensor_raw_value_x_len);

        index = index + UI_Temp_Sensor_series_column[column_index].sensor_raw_value_x_len;

        osal_memcpy((uint8 *)param[index], (uint8 *)UI_Temp_Sensor_series_column[column_index].sensor_column_width,
                    UI_Temp_Sensor_series_column[column_index].sensor_column_width_len);

        index = index + UI_Temp_Sensor_series_column[column_index].sensor_column_width_len;

        osal_memcpy((uint8 *)param[index], (uint8 *)UI_Temp_Sensor_series_column[column_index].sensor_raw_value_y,
                    UI_Temp_Sensor_series_column[column_index].sensor_raw_value_y_len);

        index = index + UI_Temp_Sensor_series_column[column_index].sensor_raw_value_y_len;
...
static API_RESULT UI_Sensor_servel_model_Sensor_ColumSeries_get(MS_ACCESS_SENSOR_MODEL_STATE_PARAMS *state_param, UINT16 id, uint8 *param, UINT16 *p_len)
{
    ...
    case TEMPERATURE_PID:
    {
        if (state_param->len > 2)
        {
            for (i = 0; i < TEMP_VALUE_LEN; i++)
            {
                Value_x_Start |= (pValue[i + 2] << (i * 8));
                Value_x_End |= (pValue[i + 2 + TEMP_VALUE_LEN] << (i * 8));
            }
            section_flag = 1;
        }
        ...
        for (i = start_index; i < end_index; i++)
        {
            *(param + index) = (uint8)TEMP_VALUE_LEN;
            index++;
            *(param + index) = (uint8)(TEMP_VALUE_LEN >> 8);
            index++;

            osal_memcpy((uint8 *)param[index], (uint8 *)UI_Temp_Sensor_series_column[i].sensor_raw_value_x,
                        UI_Temp_Sensor_series_column[i].sensor_raw_value_x_len);

            index = index + UI_Temp_Sensor_series_column[i].sensor_raw_value_x_len;

            osal_memcpy((uint8 *)param[index], (uint8 *)UI_Temp_Sensor_series_column[i].sensor_column_width,
                        UI_Temp_Sensor_series_column[i].sensor_column_width_len);

            index = index + UI_Temp_Sensor_series_column[i].sensor_column_width_len;

            osal_memcpy((uint8 *)param[index], (uint8 *)UI_Temp_Sensor_series_column[i].sensor_raw_value_y,
                        UI_Temp_Sensor_series_column[i].sensor_raw_value_y_len);

            index = index + UI_Temp_Sensor_series_column[i].sensor_raw_value_y_len;
...
void UI_Sensor_setup_server_cadence_set(UINT16 state_t, UINT16 id, MS_ACCESS_MODEL_REQ_MSG_RAW *msg_rawData, UINT8 direction)
{
    ...
    if (id == HUMIDITY_PID)
    {
        ...
        *Sensor_CadenceSetting[0].status_trigger_delta_up = msg_rawData->data_param[index];
        ...
        index++;
        osal_memcmp(Sensor_CadenceSetting[0].fast_cadence_low, (uint8 *)msg_rawData->data_param[index], HUMITY_CADENCE_LEN);
        index += HUMITY_CADENCE_LEN;

        osal_memcmp(Sensor_CadenceSetting[0].fast_cadence_high, (uint8 *)msg_rawData->data_param[index], HUMITY_CADENCE_LEN);
        index += HUMITY_CADENCE_LEN;
    }
    else if (id == TEMPERATURE_PID)
    {
        divisor_and_type = msg_rawData->data_param[index];
        ...
        *Sensor_CadenceSetting[1].status_trigger_delta_up = msg_rawData->data_param[index];
        ...
        index++;
        osal_memcmp(Sensor_CadenceSetting[1].fast_cadence_low, (uint8 *)msg_rawData->data_param[index], TEMP_CADENCE_LEN);
        index += TEMP_CADENCE_LEN;

        osal_memcmp(Sensor_CadenceSetting[1].fast_cadence_high, (uint8 *)msg_rawData->data_param[index], TEMP_CADENCE_LEN);
        index += TEMP_CADENCE_LEN;
    }
}
...
static API_RESULT UI_Sensor_setup_servel_model_settings_get(UINT16 state_t, UINT16 id, uint8 *param, UINT16 *p_len)
{
    ...
    {
        cnt = 0;
        *(param + cnt) = temp_sensor_settings.sensor_property_id;
        cnt++;
        *(param + cnt) = temp_sensor_settings.sensor_property_id >> 8;
        cnt++;
        osal_memcmp((uint8 *)param[cnt], (uint8 *)temp_sensor_settings.setting_property_ids, sizeof(UINT16) * temp_sensor_settings.setting_property_ids_count);
        cnt += sizeof(UINT16) * temp_sensor_settings.setting_property_ids_count;
        *p_len = cnt;
    }
...
static API_RESULT UI_Sensor_setup_servel_model_setting_get(MS_ACCESS_SENSOR_MODEL_STATE_PARAMS *state_param, UINT16 id, uint8 *param, UINT16 *p_len)
{
    ...
    else if (id == TEMPERATURE_PID)
    {
        for (sensor_index = 0; sensor_index < SENSOR_SETTINGS_NUM; sensor_index++)
        {
            if (setting_pid == temp_sensor_setting[sensor_index].sensor_setting_property_id)
                break;
        }

        cnt = 0;
        *(param + cnt) = (uint8)TEMPERATURE_PID;
        cnt++;
        *(param + cnt) = TEMPERATURE_PID >> 8;
        cnt++;
        *(param + cnt) = setting_pid;
        cnt++;
        *(param + cnt) = setting_pid >> 8;
        cnt++;
        *(param + cnt) = temp_sensor_setting[sensor_index].sensor_setting_access;

        osal_memcmp((uint8 *)param[cnt], (uint8 *)temp_sensor_setting[sensor_index].sensor_setting_raw, sizeof(uint8) * temp_sensor_setting[sensor_index].sensor_setting_raw_len);
        cnt += sizeof(uint8) * temp_sensor_setting[sensor_index].sensor_setting_raw_len;
...
static API_RESULT UI_Sensor_setup_servel_model_setting_set(MS_ACCESS_SENSOR_MODEL_STATE_PARAMS *state_param, UINT16 id, uint8 *param, UINT8 direct)
{
    ...
    else if (id == TEMPERATURE_PID)
    {
        ...
        osal_memcmp((uint8 *)temp_sensor_setting[sensor_index].sensor_setting_raw, (uint8 *)pValue[4],
                    sizeof(uint8) * temp_sensor_setting[sensor_index].sensor_setting_raw_len);
    }
    ...
    /* IN */ void *context)
{
    ...
    case MS_SCENE_EVENT_DELETE:
    {
        ...
        if (state_p != NULL && (state_p->index == index))
        {
            EM_free_mem(state_p);
            LOG("scene %d deleted\r\n", index);
        }
